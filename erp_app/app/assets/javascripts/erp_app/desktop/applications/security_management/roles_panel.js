					    ...
					    if (e.getKey() == e.ENTER) {
					        ...
					        button.fireEvent('click', button);
			        ...
			        },
                                {
                                    xtype: 'hidden',
                                    name: 'internal_identifier',
                                    value: selection.get('internalIdentifier')
                                }
                            ]
			}),
			buttons: [{
				...
				'click': function(button) {
					...
					success: function(form, action) {
						...
						all.getStore().load({
                                                    callback: function(){
                                                        // There is a bug in ExtJS 4 which doesn't update the selection model
                                                        // for a grid when the store is reloaded, this is a work around which
                                                        // deselect all selects the row which selected before the store load 
                                                        var index = all.getStore().find(
                                                            'internalIdentifier',
                                                            selection.get('internalIdentifier')
                                                        );
                                                        all_roles.getSelectionModel().deselectAll();
                                                        all_roles.getSelectionModel().select(index);
                                                    }
                                                });
						newWindow.close();
					    } else {
