...
(function(root, doc) {
    ...
    var parentDoc = root.parent.document;
    ...
    function initToolbar(ctx) {
        ...
        if (icons) {
            ...
            parentDoc.body.appendChild(ctx._menu);
        }
    ...
    function initEvents(ctx) {
        ...
        if (ctx._menu) {
            ...
            addListener(ctx, editor, 'mousedown', function() {
                selecting = true;
                utils.forEach(parentDoc.querySelectorAll('iframe'), function(iframe) {
                    if (iframe.contentWindow != root) {
                        iframe.contentWindow.getSelection().removeAllRanges();
                        toggleNode(iframe.contentWindow.__pen__._menu, true);
                    }
                });
            });
    ...
    Pen.prototype.menu = function() {
        ...
        var iframeOffset = this._parentWindow.document.getElementById(this._iframeId).getBoundingClientRect();
        var offset = this._range.getBoundingClientRect()
        , menuPadding = 10
        , top = iframeOffset.top + offset.top - menuPadding
        , left = iframeOffset.left + offset.left + (offset.width / 2)
        , menu = this._menu
        , menuOffset = {x: 0, y: 0}
    ...
    Pen.prototype.setIframeId = function(iframeId) {
